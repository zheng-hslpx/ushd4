"""
task_farthest_distant_first.py
结合了 Farthest Task First 策略和环境固定/序列化功能。
支持从指定路径加载固定环境。
"""
import random
import math
...
# =========================================================
# Planner（修改USV选择逻辑和任务排序）
# =========================================================
class FarthestTaskFirstPlanner: # 修改类名以反映策略
    ...
    def plan(self):
        ...
        # === 修改：按距离基地从远到近排序 (远任务优先) ===
        tasks.sort(key=lambda t: self._task_distance_from_base(t), reverse=True)

        # === 修改：USV选择逻辑 ===
        # 移除“低电量优先”，改为按USV编号（ID）顺序选择（顺序稳定，不考虑电量）
        for task in tasks:
            candidate_usvs = sorted(
                self.env.usvs,
                key=lambda u: u.usv_id  # 仅按ID排序
            )
            assigned = False
...
# =========================================================
# Multi-run orchestration (修改加载逻辑)
# =========================================================
def run_multiple(num_runs: int = NUM_RUNS):
    ...
    # 2. 依次规划
    print(f"Running {num_runs} iterations using Farthest Task First strategy...") # 修改提示信息
    for i, env in enumerate(envs):
        # === 修改：使用新的 FarthestTaskFirstPlanner ===
        planner = FarthestTaskFirstPlanner(env, log_enabled=False)
        planner.plan()
