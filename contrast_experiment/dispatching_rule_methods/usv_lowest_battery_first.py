...
# === 修改：Planner 类名以反映新策略 ===
class LowestBatteryFirstPlanner: # 原名 SimpleRandomPlanner
    ...
    # === 修改：plan 方法实现 Lowest Battery First 策略 ===
    def plan(self):
        ...
        for task in tasks:
            ...
            for usv in candidate_usvs:
                if usv.can_execute(task, self.params, self.energy_model):
                    ...
                    # 注意：这里的逻辑与原文件略有不同，原文件在充电失败后立即break。
                    # 这里我们更严格地尝试所有USV，即使前面的充电失败了。
                    # 如果需要严格按原文件逻辑，可以在这里break。
...
def run_multiple(num_runs: int = NUM_RUNS):
    ...
    # 2. 依次规划
    print(f"Running {num_runs} iterations using Lowest Battery First strategy...")
    for i, env in enumerate(envs):
        # === 修改：使用新的 LowestBatteryFirstPlanner ===
        planner = LowestBatteryFirstPlanner(env, log_enabled=False)
        planner.plan()
